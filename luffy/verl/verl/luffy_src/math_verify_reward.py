...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput, prefix_response_str=None, suffix_response_str=None) -> RewardOutput:
        assert input.problem_type == RewardType.MATH, \
            "Invalid problem type: expected 'MATH', but got '{}'".format(input.problem_type)
        
        problem = input.problem
        model_response = input.model_response
        answer = input.ground_truth["answer"]
        ...
        predict_answers = list(map(parse, [model_solution,]))
        golden_answers = list(map(parse, ["$" + input.ground_truth["answer"] + "$"] * len([model_solution,])))
        labels = list(map(verify, golden_answers, predict_answers))
        ...
        if random.random() < 0.05:
            ...
            if problem is not None:
                if random.random() < 0.2:
                    print(f"\n[Prompt]\n{problem}")
            if prefix_response_str is not None and suffix_response_str is not None:
                print(f"\n[Model Response]\n[Off-policy Response]\n{prefix_response_str}")
                print(f"\n[On-policy Response]\n{suffix_response_str}")
            else:
                print(f"\n[Model Response]\n{model_response}")
            print(f"\n[Ground Truth]\n{answer}")
            box_match = 1 if labels[0] is True else 0
            print(f"\n[Reward Score]\n{box_match}")
...
def reward_fn_math_verify_no_think(prompt_str: str, solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False, prefix_response_str=None, suffix_response_str=None):
    reward_config = RewardConfig()
    reward_config.think_format = False
    reward_config.use_math_orm = enable_llm
    reward_fn = RewardMathFn(reward_config)
    reward_response = reward_fn(RewardInput(problem=prompt_str, problem_type=RewardType.MATH, model_response=solution_str, ground_truth={"answer": ground_truth}), prefix_response_str=prefix_response_str, suffix_response_str=suffix_response_str)
    return reward_response.is_correct
