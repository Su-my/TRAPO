...
def _get_rank_id():
    # 依次尝试常见环境变量；都没有就当成单卡=0
    for k in ["RANK", "WORLD_RANK", "SLURM_PROCID", "LOCAL_RANK"]:
...
def _append_step_token_probs(step: int,
                             off_probs_1d: np.ndarray,
                             on_probs_1d: np.ndarray,
                             log_dir: str = "/home/sumingyu/log",
                             rank: int | None = None):
    """
    将当前 microbatch 的 off/on token 概率追加到本 step、当前 rank 的 npz 文件中。
    文件结构：npz，包含两个 1D 数组：off, on
    """
    os.makedirs(log_dir, exist_ok=True)
    if rank is None:
        rank = _get_rank_id()
    path = os.path.join(log_dir, f"token_probs_step{step}_rank{rank}.npz")
    ...
    if os.path.exists(path):
        try:
            ...
            old_off = data["off"]
            old_on  = data["on"]
            new_off = np.concatenate([old_off, off_probs_1d])
            new_on  = np.concatenate([old_on,  on_probs_1d])
        except Exception:
            # 若旧文件损坏，保底从当前数据重建
            new_off = off_probs_1d
            new_on  = on_probs_1d
    else:
        new_off = off_probs_1d
        new_on  = on_probs_1d
    ...
    # 原子覆盖写回（防止部分写入）
    with tempfile.NamedTemporaryFile(dir=log_dir, delete=False, suffix=".npz") as tmp:
        np.savez(tmp.name, off=new_off, on=new_on)
        tmp_path = tmp.name
...
def compute_token_on_off_policy_loss(
    ...
    if target_probs is None:
        ...
        elif off_policy_reshape == "clip_low":
            off_ratio = torch.exp(log_prob) / torch.clamp(torch.exp(log_prob.detach()), min=0.1)
        elif off_policy_reshape == "clip_low_and_shift":
            off_ratio = torch.exp(log_prob) * ( 1 / torch.clamp(torch.exp(log_prob.detach()), min=0.1) - 0.5 )
        else:
    ...
    off_ratio_mean = verl_F.masked_mean(off_ratio, off_loss_mask * prefix_mask * eos_mask)
    if off_ratio_mean.isnan().any().item():
        off_ratio_mean = torch.tensor(0.0)

    off_pg_losses = -off_advantages * off_ratio
    off_pg_loss = verl_F.masked_mean(off_pg_losses, off_loss_mask * prefix_mask * eos_mask)
    ...
    pg_losses = off_pg_losses * (off_loss_mask * prefix_mask) + on_pg_losses * (1 - prefix_mask)
    ...
    with torch.no_grad():
        token_probs = torch.exp(log_prob).detach().cpu()
        valid_mask  = eos_mask.detach().cpu().bool()
        prefix_cpu  = prefix_mask.detach().cpu().bool()

        off_flat = token_probs[prefix_cpu & valid_mask].reshape(-1).numpy()
        on_flat  = token_probs[(~prefix_cpu) & valid_mask].reshape(-1).numpy()

        _append_step_token_probs(step=global_steps,
                                off_probs_1d=off_flat,
                                on_probs_1d=on_flat,
                                rank=_get_rank_id())
