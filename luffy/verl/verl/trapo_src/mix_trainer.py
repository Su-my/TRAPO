...
class MIXRayPPOTrainer(RayPPOTrainer):
    ...
    def _create_dataloader(self):
        ...
        from .rl_dataset_with_target import RLHFDatasetWithTargetTrain, RLHFDatasetWithTargetTest
        self.train_dataset = RLHFDatasetWithTargetTrain(parquet_files=self.config.data.train_files,
        ...
        if self.config.data.shuffle:
            from verl.trapo_src.rl_dataset_with_target import ResumableRandomSampler
        ...
        self.val_dataset = RLHFDatasetWithTargetTest(parquet_files=self.config.data.val_files,
                                       tokenizer=self.tokenizer,
                                       prompt_key=self.config.data.prompt_key,
                                       max_prompt_length=self.config.data.max_prompt_length,
                                       max_response_length=self.config.data.max_response_length,
    ...
    def _save_checkpoint(self):
        ...
        try:
            ...
            for each_save_step in prev_save_steps:
                if each_save_step >= 350 and each_save_step <= 600:
                    # keep all checkpoints between 400 and 600
                    continue
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids', 'hint_input_ids_0', 'hint_input_ids_1'])
                gen_batch.meta_info['global_steps'] = self.global_steps

                with _timer('step', timing_raw):
                    gen_batch.meta_info["rollout_n"] = 4
                    gen_batch.meta_info["batch_id"] = 1
                    hint_levels = np.array([0] * len(gen_batch), dtype=np.int32)
                    gen_batch.batch["hint_levels"] = torch.tensor(hint_levels, dtype=torch.int32).view(-1, 1)
                    total_score = np.array([0.0] * len(gen_batch), dtype=np.float32)
                    gen_batch.batch["total_scores"] = torch.tensor(total_score, dtype=torch.float32).view(-1, 1)
                    gen_batch_output1 = self.actor_rollout_wg.generate_sequences(gen_batch)
                    gen_batch_output1.meta_info.pop("timing", None)
                    batch1 = batch.repeat(repeat_times=4, interleave=True)
                    batch1 = batch1.union(gen_batch_output1)
                    reward_tensor1 = self.reward_fn(batch1)
                    batch1.batch['token_level_scores'] = reward_tensor1
                    scores1 = reward_tensor1.sum(dim=-1).cpu().tolist()
                    for group_i in range(len(gen_batch)):
                        group_sum = sum(scores1[group_i * 4:(group_i + 1) * 4])
                        gen_batch.batch["total_scores"][group_i] += group_sum
                        if gen_batch.batch["total_scores"][group_i] < 2:
                            gen_batch.batch["hint_levels"][group_i] = 1
                        else:
                            gen_batch.batch["hint_levels"][group_i] = 0


                    gen_batch.meta_info["rollout_n"] = 2
                    gen_batch.meta_info["batch_id"] = 2
                    gen_batch_output2 = self.actor_rollout_wg.generate_sequences(gen_batch)
                    gen_batch_output2.meta_info.pop("timing", None)
                    batch2 = batch.repeat(repeat_times=2, interleave=True)
                    batch2 = batch2.union(gen_batch_output2)
                    reward_tensor2 = self.reward_fn(batch2)
                    batch2.batch["token_level_scores"] = reward_tensor2
                    scores2 = reward_tensor2.sum(dim=-1).cpu().tolist()
                    for group_i in range(len(gen_batch)):
                        group_sum = sum(scores2[group_i * 2:(group_i + 1) * 2])
                        gen_batch.batch["total_scores"][group_i] += group_sum
                        if gen_batch.batch["total_scores"][group_i] < 4:
                            gen_batch.batch["hint_levels"][group_i] = 2
                        else:
                            gen_batch.batch["hint_levels"][group_i] = 0


                    gen_batch.meta_info["rollout_n"] = 1
                    gen_batch.meta_info["batch_id"] = 3
                    gen_batch_output3 = self.actor_rollout_wg.generate_sequences(gen_batch)
                    gen_batch_output3.meta_info.pop("timing", None)
                    batch3 = batch.repeat(repeat_times=1, interleave=True)
                    batch3 = batch3.union(gen_batch_output3)
                    reward_tensor3 = self.reward_fn(batch3)
                    batch3.batch["token_level_scores"] = reward_tensor3
                    scores3 = reward_tensor3.sum(dim=-1).cpu().tolist()
                    for group_i in range(len(gen_batch)):
                        group_sum = scores3[group_i]
                        gen_batch.batch["total_scores"][group_i] += group_sum
                        if gen_batch.batch["total_scores"][group_i] < 7:
                            gen_batch.batch["hint_levels"][group_i] = 3
                        else:
                            gen_batch.batch["hint_levels"][group_i] = 0


                    gen_batch.meta_info["rollout_n"] = 1
                    gen_batch.meta_info["batch_id"] = 4
                    gen_batch_output4 = self.actor_rollout_wg.generate_sequences(gen_batch)
                    gen_batch_output4.meta_info.pop("timing", None)
                    batch4 = batch.repeat(repeat_times=1, interleave=True)
                    batch4 = batch4.union(gen_batch_output4)
                    reward_tensor4 = self.reward_fn(batch4)
                    batch4.batch["token_level_scores"] = reward_tensor4
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        hint_level_0_num = (batch.batch["hint_levels"] == 0).sum().item()
                        hint_level_1_num = (batch.batch["hint_levels"] == 1).sum().item()
                        hint_level_2_num = (batch.batch["hint_levels"] == 2).sum().item()
                        hint_level_3_num = (batch.batch["hint_levels"] == 3).sum().item()
                        print(f"Hint levels: 0: {hint_level_0_num}, 1: {hint_level_1_num}, 2: {hint_level_2_num}, 3: {hint_level_3_num}")
                        metrics['batch/hint_level_0'] = hint_level_0_num / len(uids)
                        metrics['batch/hint_level_1'] = hint_level_1_num / len(uids)
                        metrics['batch/hint_level_2'] = hint_level_2_num / len(uids)
                        metrics['batch/hint_level_3'] = hint_level_3_num / len(uids)
                        
                        off_loss_mask = batch.batch["off_loss_mask"]
                        off_loss_seq_mask = off_loss_mask.any(-1)
                        total_off = off_loss_seq_mask.sum().item()
                        print(f"Total off policy samples: {total_off}")
                    ...
                    elif self.global_steps >= 400 and self.global_steps <= 600 and self.global_steps % self.config.trainer.test_freq == 0:
                        with _timer('save_checkpoint', timing_raw):
                            self._save_checkpoint()
...
def compute_data_metrics_ours(batch, use_critic=True):
    ...
    group_index = batch.batch["group_index"]
    mask1 = group_index == 1
    mask2 = group_index == 2
    ...
    metrics = {
        ...
        "critic/score/mask1_mean": torch.mean(sequence_score[mask1]).detach().item(),
        # reward
        'critic/rewards/mean':
