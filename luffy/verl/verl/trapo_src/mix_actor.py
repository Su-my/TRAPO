...
class MIXDataParallelPPOActor(DataParallelPPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'position_ids', 'old_log_probs', 'advantages', 'prefix_mask', 'off_loss_mask']
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    off_loss_mask = data['off_loss_mask']
                    # advantages_of_hint = torch.clamp(advantages, min=0)
                    advantages_of_hint = torch.ones_like(advantages)
